...
function checkForEmptyRoutes(context, opts) {
  if (!context.express._router) {
    context.express.lazyrouter();
  }

  if (context.express._router.stack.length > 2) { return; }

  context.express.use(function(req, res) {
    res.redirect('https://github.com/app-context/app-context-express');
    // express.response.render.call(res, path.join(__dirname, 'empty-router.html'));
  });
}
